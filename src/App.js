...
function AppContent(){
  ...
  const loadData=useCallback(async(uid)=>{
    const{data:p}=await supabase.from('profils').select('*').eq('user_id',uid).maybeSingle();
    if(p){setProfil({nom:p.nom||'',entreprise:p.entreprise||'',siren:p.siren||'',siret:p.siret||'',kbis:p.kbis||'',tel:p.tel||'',whatsapp:p.whatsapp||'',snap:p.snap||'',email:p.email||'',adresse:p.adresse||'',ville:p.ville||'',iban:p.iban||''});}else{setProfil(INIT_PROFIL);}
    const{data:vs}=await supabase.from('vehicles').select('*').eq('user_id',uid).order('created_at',{ascending:false});
    if(vs)setVehicles(vs.map(v=>({id:v.id,marque:v.marque,modele:v.modele,immat:v.immat,couleur:v.couleur||'',annee:v.annee||'',km:v.km||0,tarif:v.tarif||0,caution:v.caution||1000,kmInclus:v.km_inclus||0,prixKmSup:v.prix_km_sup||0,kmIllimite:v.km_illimite||false,docs:v.docs||[],frais:v.frais||DEF_FRAIS,clauses:v.clauses||DEF_CLAUSES,tarifsSpeciaux:v.tarifs_speciaux||[]})));
    const{data:cs}=await supabase.from('contrats').select('*').eq('user_id',uid).order('created_at',{ascending:false});
    if(cs)setContrats(cs.map(c=>({id:c.id,locNom:c.loc_nom||'',locAdresse:c.loc_adresse||'',locTel:c.loc_tel||'',locEmail:c.loc_email||'',locPermis:c.loc_permis||'',dateDebut:c.date_debut||'',heureDebut:c.heure_debut||'10:00',dateFin:c.date_fin||'',heureFin:c.heure_fin||'10:00',paiement:c.paiement||'especes',cautionMode:c.caution_mode||'especes',kmDepart:c.km_depart||'',nbJours:c.nb_jours||1,heuresLoc:c.heures_loc||24,carburantDepart:c.carburant_depart||100,exterieurPropre:c.exterieur_propre,interieurPropre:c.interieur_propre,vehicleId:c.vehicle_id,vehicleLabel:c.vehicle_label||'',immat:c.immat||'',sigL:c.sig_l,sigLoc:c.sig_loc,totalCalc:c.total_calc||0,tarifLabel:c.tarif_label||'',photosDepart:c.photos_depart||[],fraisSnap:c.frais_snap||[],clausesSnap:c.clauses_snap||[],kmInclus:c.km_inclus||0,prixKmSup:c.prix_km_sup||0})));
    const{data:ds}=await supabase.from('depenses').select('*').eq('user_id',uid).order('created_at',{ascending:false});
    if(ds)setDepenses(ds.map(d=>({id:d.id,label:d.label||'',montant:d.montant||0,categorie:d.categorie||'Carburant',date:d.date||'',vehicleId:d.vehicle_id||''})));
    const{data:rs}=await supabase.from('retours').select('*').eq('user_id',uid);
    if(rs){const map={};rs.forEach(r=>{map[r.contrat_id]={checks:r.checks||{},carro:r.carro||{},carroPhotos:r.carro_photos||{},carroNotes:r.carro_notes||{},photos:r.photos||{},notes:r.notes||{},cautionRestituee:r.caution_restituee,montantRetenu:r.montant_retenu||0,raisonRetenue:r.raison_retenue||'',rembourse:r.rembourse||0,kmRetour:r.km_retour||'',kmSup:r.km_sup||0,surplusKm:r.surplus_km||0,carburantRetour:r.carburant_retour||100,date:r.date};});setRetours(map);}
  },[]);

  useEffect(()=>{
    supabase.auth.getSession().then(({data:{session}})=>{const u=session?.user||null;setUser(u);if(u)loadData(u.id);});
    const{data:{subscription}}=supabase.auth.onAuthStateChange((_,session)=>{const u=session?.user||null;setUser(u);if(u)loadData(u.id);});
  ...
  async function saveContrat(){
    ...
    if(user)await supabase.from('contrats').insert({id:c.id,user_id:user.id,vehicle_id:c.vehicleId,vehicle_label:c.vehicleLabel,immat:c.immat,loc_nom:c.locNom,loc_adresse:c.locAdresse,loc_tel:c.locTel,loc_email:c.locEmail,loc_permis:c.locPermis,date_debut:c.dateDebut,heure_debut:c.heureDebut,date_fin:c.dateFin,heure_fin:c.heureFin,paiement:c.paiement,caution_mode:c.cautionMode,km_depart:c.kmDepart,nb_jours:c.nbJours,heures_loc:c.heuresLoc,carburant_depart:c.carburantDepart,exterieur_propre:c.exterieurPropre,interieur_propre:c.interieurPropre,total_calc:c.totalCalc,tarif_label:c.tarifLabel,sig_l:c.sigL,sig_loc:c.sigLoc,photos_depart:c.photosDepart,frais_snap:c.fraisSnap,clauses_snap:c.clausesSnap,km_inclus:c.kmInclus,prix_km_sup:c.prixKmSup});
  }
  ...
  async function saveRetour(contratId,data){
    ...
    if(ct&&data.kmRetour){
      ...
      if(user)await supabase.from('vehicles').update({km:parseFloat(data.kmRetour),updated_at:new Date().toISOString()}).eq('id',ct.vehicleId).eq('user_id',user.id);
    }
    toast_("✅ Retour enregistré !");setRetourContratId(null);
    if(user)await supabase.from('retours').upsert({user_id:user.id,contrat_id:contratId,checks:data.checks,carro:data.carro,carro_photos:data.carroPhotos,carro_notes:data.carroNotes,photos:data.photos,notes:data.notes,caution_restituee:data.cautionRestituee,montant_retenu:data.montantRetenu,raison_retenue:data.raisonRetenue,rembourse:data.rembourse,km_retour:data.kmRetour,km_sup:data.kmSup,surplus_km:data.surplusKm,carburant_retour:data.carburantRetour,date:data.date},{onConflict:'contrat_id'});
  }
  ...
  async function addV(){
    ...
    if(editV){
      ...
      if(user)await supabase.from('vehicles').update({marque:base.marque,modele:base.modele,immat:base.immat,couleur:base.couleur,annee:base.annee,km:base.km,tarif:base.tarif,caution:base.caution,km_inclus:base.kmInclus,prix_km_sup:base.prixKmSup,km_illimite:base.kmIllimite,updated_at:new Date().toISOString()}).eq('id',editV.id).eq('user_id',user.id);
    } else {
      const newId=Date.now();
      const newV={id:newId,...base,docs:[],frais:DEF_FRAIS.map(f=>({...f})),clauses:DEF_CLAUSES.map(c=>({...c})),tarifsSpeciaux:[]};
      setVehicles(vs=>[...vs,newV]);toast_("Véhicule ajouté !");
      if(user)await supabase.from('vehicles').insert({id:newId,user_id:user.id,marque:base.marque,modele:base.modele,immat:base.immat,couleur:base.couleur,annee:base.annee,km:base.km,tarif:base.tarif,caution:base.caution,km_inclus:base.kmInclus,prix_km_sup:base.prixKmSup,km_illimite:base.kmIllimite,docs:[],frais:DEF_FRAIS,clauses:DEF_CLAUSES,tarifs_speciaux:[]});
    }
  ...
  async function saveTarifs(){
    ...
    if(user)await supabase.from('vehicles').update({tarifs_speciaux:[...tarifsTemp],updated_at:new Date().toISOString()}).eq('id',tarifsVehicleId).eq('user_id',user.id);
          ...
          <div style={{background:"white",borderRadius:18,width:"100%",maxWidth:600,maxHeight:"88vh",display:"flex",flexDirection:"column"}}>
            ...
            <div style={{overflowY:"auto",flex:1,padding:14}}>
              <div style={{background:"#eff6ff",borderRadius:10,padding:12,marginBottom:12,border:"1px solid #bfdbfe"}}>
                ...
                <button onClick={async()=>{if(!newDoc.nom){toast_("Donnez un nom","error");return;}const updatedDocs=[...(docsV.docs||[]),{id:Date.now(),...newDoc}];setVehicles(vs=>vs.map(v=>v.id===docsId?{...v,docs:updatedDocs}:v));setNewDoc({type:"Carte grise",nom:"",expiration:"",file:null,fileData:null});toast_("Document ajouté !");if(user)await supabase.from('vehicles').update({docs:updatedDocs,updated_at:new Date().toISOString()}).eq('id',docsId).eq('user_id',user.id);}} style={{marginTop:8,background:"#2563eb",color:"white",border:"none",borderRadius:7,padding:"7px 14px",fontSize:12,fontWeight:700,cursor:"pointer"}}>Enregistrer</button>
              </div>
                ...
                <div key={v.id} style={{background:"white",borderRadius:16,boxShadow:"0 2px 10px rgba(0,0,0,.08)",overflow:"hidden",border:"1px solid #e5e7eb"}}>
                  ...
                  <div style={{padding:14}}>
                    ...
                    <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                      ...
                      <button onClick={async()=>{if(window.confirm("Supprimer ?")){setVehicles(vs=>vs.filter(x=>x.id!==v.id));if(user)await supabase.from('vehicles').delete().eq('id',v.id).eq('user_id',user.id);}}} style={{padding:"6px 10px",background:"#fef2f2",color:"#dc2626",border:"1px solid #fecaca",borderRadius:8,fontSize:11,cursor:"pointer"}}>🗑️</button>
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div style={{background:"white",borderRadius:14,padding:16,marginBottom:14,boxShadow:"0 2px 8px rgba(0,0,0,.07)"}}>
              ...
              {showAddD&&(
                <div style={{background:"#f8fafc",borderRadius:10,padding:12,marginBottom:12,border:"1px solid #e5e7eb"}}>
                  ...
                  <button onClick={async()=>{if(!dForm.label||!dForm.montant){toast_("Remplissez libellé et montant","error");return;}const newId=Date.now();const nd={id:newId,...dForm};setDepenses(d=>[nd,...d]);setDForm({label:"",montant:"",categorie:"Carburant",date:new Date().toISOString().slice(0,10),vehicleId:""});setShowAddD(false);toast_("Dépense ajoutée");if(user)await supabase.from('depenses').insert({id:newId,user_id:user.id,vehicle_id:nd.vehicleId||null,label:nd.label,montant:parseFloat(nd.montant),categorie:nd.categorie,date:nd.date});}} style={{background:"#16a34a",color:"white",border:"none",borderRadius:8,padding:"7px 14px",fontSize:12,fontWeight:700,cursor:"pointer"}}>Enregistrer</button>
                </div>
              )}
              {depenses.length===0
